...
DEFAULT_DATA_PATH = Path("./data/prp_mktx")
DEFAULT_FIRE_PATH = Path("./data/inc_utm32")
DEFAULT_RESULTS_PATH = Path("./results")
COMPONENT_NAMES = ("A", "U", "Sy", "Sm", "Ry", "Rm")


def load_land_cover(year: int, data_path: Path) -> Tuple[np.ndarray, Affine, CRS | None]:
...
def load_fire_mask(year: int, fire_path: Path, expected_shape: Tuple[int, int]) -> np.ndarray:
...
def save_encoded_grid(encoded: np.ndarray, transform: Affine | None, crs: CRS | None, year: int, results_path: Path) -> None:
...
def run_simulation(
    ...
    fire_path: Path = DEFAULT_FIRE_PATH,
    results_path: Path = DEFAULT_RESULTS_PATH,
) -> None:
    """Execute the full simulation for the selected time window."""
    start_time = time.perf_counter()

    land_cover, transform, crs = load_land_cover(start_year, data_path)
    ...
    for idx, year in enumerate(years):
        print(year)
        fire_mask = load_fire_mask(year, fire_path, grid_shape)
        step_stats = model.step(fire_mask)
        system_metrics[idx] = step_stats["averages"]
        save_encoded_grid(model.encode_grid(), transform, crs, year, results_path)
